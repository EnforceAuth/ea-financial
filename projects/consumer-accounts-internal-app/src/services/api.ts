...
class ApiService {
  ...
  private handleApiError(error: AxiosError): Error {
    if (error.response?.data) {
      ...
      // Handle OPA service unavailability specifically
      if (
        error.response.status === 503 &&
        apiError.message?.includes("Authorization service unavailable")
      ) {
        return new Error(
          "🔒 Authorization system is currently unavailable. The banking security service (OPA) is not running. Please contact your system administrator or try again later.",
        );
      }
      ...
      if (error.response.status === 401 || error.response.status === 403) {
        if (
          apiError.error?.includes("OPA service") ||
          apiError.error?.includes("Token validation service unavailable")
        ) {
          return new Error(
            "🔒 Unable to verify your credentials. The authorization service is not available. Please try again later.",
          );
        }
      }
  ...
  async getEmployeeProcedures(): Promise<any> {
    ...
    throw new Error(
      response.data.message || "Failed to fetch employee procedures",
    );
  }
  ...
  async getRegulatoryDisclosures(): Promise<any> {
    ...
    throw new Error(
      response.data.message || "Failed to fetch regulatory disclosures",
    );
  }
  ...
  async getAccountPolicies(): Promise<any> {
    ...
    throw new Error(
      response.data.message || "Failed to fetch account policies",
    );
  }
  ...
  async getTransactionLimits(): Promise<any> {
    ...
    throw new Error(
      response.data.message || "Failed to fetch transaction limits",
    );
  }

...
export const termsRoutes = new Elysia({ prefix: '/terms' })
  .get('/', async ({ headers, set }): Promise<ApiResponse<TermsResponse>> => {
    try {
      ...
      return {
        ...
        data: {
          terms: termsData as unknown as TermsData,
        },
      };
    } catch (_error) {
      set.status = 500;
  ...
  .get('/general', async ({ headers, set }): Promise<ApiResponse<TermsResponse>> => {
    try {
      ...
      return {
        ...
        data: {
          terms: termsData.general_terms as TermsData,
        },
      };
    } catch (_error) {
      set.status = 500;
  ...
  .get('/regulatory', async ({ headers, set }): Promise<ApiResponse<TermsResponse>> => {
    try {
      ...
      return {
        ...
        data: {
          terms: termsData.regulatory_disclosures as TermsData,
        },
      };
    } catch (_error) {
      set.status = 500;
  ...
  .get('/account-policies', async ({ headers, set }): Promise<ApiResponse<unknown>> => {
    try {
      ...
      const termsData = dataService.getTermsData();
      const generalTerms = termsData.general_terms as
        | { sections?: { account_policies?: unknown } }
        | undefined;

      return {
        success: true,
        message: 'Account policies retrieved successfully',
        data: generalTerms?.sections?.account_policies,
      };
    } catch (_error) {
      set.status = 500;
      return {
  ...
  .get('/transaction-limits', async ({ headers, set }): Promise<ApiResponse<unknown>> => {
    try {
      ...
      const termsData = dataService.getTermsData();
      const generalTerms = termsData.general_terms as
        | { sections?: { transaction_limits?: unknown } }
        | undefined;

      return {
        success: true,
        message: 'Transaction limits retrieved successfully',
        data: generalTerms?.sections?.transaction_limits,
      };
    } catch (_error) {
      set.status = 500;
      return {

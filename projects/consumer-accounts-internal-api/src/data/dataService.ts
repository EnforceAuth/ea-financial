...
class DataService {
  ...
  private loadData() {
    try {
      ...
      console.log('Warning: Could not load fixture data, continuing with empty datasets');
    }
  }
  ...
  getTotalTransactionCount(accountId: string): number {
    return this.transactions.filter(txn => txn.accountId === accountId).length;
  }

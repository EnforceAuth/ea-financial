...
const _app = new Elysia()
  ...
  .get('/', () => ({
    name: appName,
    version: '1.0.0',
    description: 'Internal API for EA Financial consumer account operations with OPA authorization',
    endpoints: {
      authentication: {
...
opaService.healthCheck().then(health => {
  if (health.healthy) {
    console.log('OPA service is healthy and connected');
  } else {
    console.log('OPA service connection failed - continuing without OPA authorization');
  }
});

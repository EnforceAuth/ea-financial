...
describe("Account Routes", () => {
  ...
  beforeAll(async () => {
    ...
    // Get manager token
    const managerResponse = await app
      .handle(
        new Request("http://localhost/auth/login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    const limitedResponse = await app
      .handle(
        new Request("http://localhost/auth/login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  describe("POST /accounts/:accountId/debit", () => {
    it("should successfully process debit transaction with sufficient funds", async () => {
      ...
      expect(data.data.transaction.amount).toBe(50.00);
  ...
  describe("POST /accounts/:accountId/credit", () => {
    it("should successfully process credit transaction", async () => {
      ...
      expect(data.data.transaction.amount).toBe(100.00);
      expect(data.data.newBalance).toBe(225.50); // 125.50 + 100.00
    });
  ...
  describe("Integration - Account Operations Flow", () => {
    it("should complete full account operation workflow", async () => {
      ...
      const balanceResponse1 = await app
        .handle(
      ...
      expect(creditData.data.newBalance).toBe(initialBalance + 500.00);

      // Step 3: Check updated balance
      const balanceResponse2 = await app
        .handle(
          new Request("http://localhost/accounts/acc_002/balance", {
            method: "GET",
            headers: {
      ...
      expect(balanceData2.data.balance).toBe(initialBalance + 500.00);

      // Step 4: View transaction history to verify the transaction
      const transactionsResponse = await app
        .handle(
